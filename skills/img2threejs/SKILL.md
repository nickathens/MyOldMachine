...
Turn a reference image of an object or character into an animation ready procedural Three.js model written as TypeScript code, built pass by pass behind vision quality gates. Reconstruction by code, not photogrammetry or mesh extraction. Use it for object rebuilds, sculpt specs, game props, material studies, and stylized character likeness. For meshes authored by hand use `blender`; for game scenes use `godot`; for web motion graphics use `remotion`; for a generated image rather than a model use `image-gen`.

---

## Running it

Every script path in this document is relative to the skill root, so run from there:

```bash
cd skills/img2threejs && python3 forge/next.py <spec.json>
```

Pure Python 3 standard library (3.10 or newer), nothing to install. The system `python3`
runs it; no venv is needed.

Two operational notes, verified by running rather than assumed:

- The pipeline entry points (`forge/next.py`, `forge/stage1_intake/probe_image.py`, and the stage scripts) run standalone from the skill root. The CS2 test files are the exception: they import `forge.` as a package, so they need `PYTHONPATH=. python3 forge/tests/test_cs2_foundation.py`.
- The `.cache/spec-search/` directory is generated on first spec search and reaches roughly 4 MB. It is gitignored upstream and here. Delete it freely; it rebuilds.

## Provenance, license, and known defects

Vendored from https://github.com/img2threejs/img2threejs at commit `f1ade81` (2026-07-26), version 1.4.1, **Apache License 2.0**. The full licence text is in `LICENSE` and must stay with these files. Everything below this line is the upstream document, kept verbatim so future updates apply as a clean diff; only this header is ours.

Pruned from the vendored copy: `.github/`, `ROADMAP.md`, `CONTRIBUTING.md` (repository scaffolding with no runtime role). Nothing executable was removed.

Verified at adoption time by running the suite, not by reading the README:

- **21 of 25 test files pass clean.** Two more (`test_cs2_foundation`, `test_cs2_manifest`) pass once `PYTHONPATH=.` is set.
- **`forge/tests/test_pipeline.py` fails 2 of its 56 tests upstream**, and `test_cs2_review.py` errors. The pipeline failure is a real upstream defect: `forge/stage4_review/append_review.py` line 283 refuses to credit the blockout pass without `--map-stripped-render`, but the test at line 572 calls it without that flag. The gate was tightened and its test was never updated. Practical effect: crediting a blockout pass requires passing `--map-stripped-render`, so supply an unlit, map stripped render at that step rather than assuming the documented call signature is complete.
- **The upstream repository has no CI**, only a funding file. Treat its green README claims as unverified and re-run the suite after any upstream pull.

A large share of `docs/` (roughly 220 KB across `docs/cs2/` and `docs/cs2-anatomy/`) is a Counter Strike 2 weapon skin vertical. It is unused here but left in place because the geometry adapters in `forge/` reference it, and removing it would break the pipeline and make upstream diffs painful.
