...
**One model re-prices when a `video_references` file is attached [live 2026-08-10, re-checked
2026-08-23], and `seedance2.5` is no longer the other one.** The attached file is what moves the
price, not the name of the mode, so the trap springs on a quote that names no mode at all:
