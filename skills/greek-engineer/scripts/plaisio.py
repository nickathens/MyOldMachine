...
2. ΠΡΟΑΙΡΕΤΙΚΗ ΔΙΑΣΤΑΥΡΩΣΗ ΜΕ ΑΝΕΞΑΡΤΗΤΟ ΕΠΙΛΥΤΗ. Η εντολή --pynite ξαναλύνει το
   ...
   γνώμη, όχι προϋπόθεση. Το PyNiteFEA ζει σε δικό του απομονωμένο venv
   (~/.venvs/engineering, ή $GREEK_ENGINEER_VENV) και η διασταύρωση γεφυρώνεται
   εκεί με υποδιεργασία. Η απομόνωση είναι πολιτική, όχι ενεργή σύγκρουση: το
   PyNiteFEA απαιτεί numpy >= 2.4, ενώ το ταβάνι numpy του numba, πάνω στο οποίο
   πατά η φωνητική μεταγραφή whisper, αλλάζει με την έκδοση του numba (η σειρά
   0.63.x όριζε numpy < 2.4, ασύμβατο, ενώ το 0.65.1 που είναι εγκατεστημένο εδώ,
   έλεγχος 2026-07-18, δέχεται numpy < 2.5 και συνυπάρχει με το numpy 2.4.4, και
   το whisper δεν καρφώνει numpy), οπότε οι βαριές μηχανές μένουν εκτός του κοινού
   venv αντί να ποντάρουν στη συμβατότητα της ημέρας.
   Στήσιμο: scripts/setup_engine_venv.sh. Αν το venv λείπει, οι κλειστοί τύποι
   ισχύουν μόνοι.
...
def _engine_python():
    ...
    που απαιτεί να μένει εκτός της βασικής εγκατάστασης, της οποίας το ταβάνι
    numpy το ορίζει η εκάστοτε έκδοση numba (πολιτική απομόνωσης, όχι ενεργή
    σύγκρουση, βλέπε την αρχή 2 στο docstring της κορυφής).
    """
