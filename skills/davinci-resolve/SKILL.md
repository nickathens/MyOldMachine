...
`resolve_api.py status` reports which situation the machine is in. Never assume; run it. On Studio it answers with the product's own name, on this machine `DaVinci Resolve Studio 21.1.0.17`, which is the only reading that settles the question. The bundle identifier and the app name are IDENTICAL on both editions, so neither can answer it.
...
- Updated on this machine to **Studio 21.1.0.17** on 2026-09-11, and the update did **not** spend an activation. This matters because the key here has **no activations left**: the worry is that an update re-activates, and it does not. The key is not version tied. The app's own log is the proof, and reading it is how to answer this rather than asking anyone: the 11 Sep launch logs `Checking Licenses` with no new `Activated lic type:1` line, where the first activation on 23 Aug logged one, and the files under `/Library/Application Support/Blackmagic Design/DaVinci Resolve/.license/` keep their original timestamps straight through the install.
- **A licence still has to be deactivated from inside Resolve before a wipe, a drive swap or a move**, or the seat is spent and it becomes a support ticket. Flag this before any OS reinstall.
- **After an update the first launch parks on a Welcome screen and never reaches the licence check.** A separate helper, `DaVinci Resolve Welcome`, lives inside the app at `Contents/Applications/.hidden/` and holds the main process before it loads. Nothing is written to `davinci_resolve.log` while it sits there, so a log grep looks exactly like a hung launch when the app is merely waiting for a click. Check `pgrep -lf "DaVinci Resolve"` whenever a launch seems to go nowhere; killing the helper pid lets the main process carry on.
- **Archive before updating**, and archive the right thing. The reversible state is tiny (the project library gzips to well under a megabyte, the licence state to a few dozen KB) and the project format is a one way door, so a newer Resolve opening an older library upgrades it with no way back. Do not bother copying the 8.8 GB app bundle: the installer writes far outside `/Applications`, so a bundle copy is only half a rollback. The real rollback is Blackmagic's own old installer, which the downloads API still serves for every past release, plus the restored library.
