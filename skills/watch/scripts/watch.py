...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Force a specific Whisper backend. Default order: Groq → OpenAI → local CLI. "
            "Local runs offline via the `whisper` CLI (slower, no API key required)."
        ),
    )
    args = ap.parse_args()
    ...
    if not transcript_segments and not args.no_whisper:
        ...
        # The local CLI returns api_key="" — treat presence of a backend as enough.
        if backend and (api_key or backend == "local"):
            try:
                trim_duration = effective_duration if focused else None
                trim_start = start_sec if focused else None
            ...
            hint = (
                ...
                "no subtitles, no Whisper API key, and no local whisper CLI found"
            )
