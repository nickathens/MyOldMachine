...
#
# Backend priority (auto-detected): Groq → OpenAI → local `whisper` CLI.
# - Groq (whisper-large-v3): fastest, cheapest API. Get a key: https://console.groq.com/keys
# - OpenAI (whisper-1): compatible fallback. Key: https://platform.openai.com/api-keys
# - Local CLI: offline, free, runs on this machine. Slow on CPU but always available.
#
# If neither API key is set and the `whisper` CLI is installed, /watch falls
# back to local transcription automatically. Force with `--whisper local`.
#
# Local-CLI tuning (uncomment and edit to override defaults):
#   WATCH_LOCAL_WHISPER_MODEL=base   # tiny | base | small | medium | large-v3 | turbo
#   WATCH_LOCAL_WHISPER_DEVICE=cpu   # cpu | cuda  (cuda needs PyTorch CUDA + supported GPU)
...
def cmd_check() -> int:
    ...
    if not s["has_api_key"]:
        parts.append("no Whisper backend (GROQ_API_KEY, OPENAI_API_KEY, or local `whisper` CLI)")
...
def cmd_install() -> int:
    ...
    print("")
    print("[setup] one step left: enable Whisper transcription.")
    print("")
    print("  Option A — install the local `whisper` CLI (offline, free):")
    print("    pip install --user openai-whisper   # or use a venv")
    print("")
    print(f"  Option B — add an API key in {CONFIG_FILE}:")
    print("    GROQ_API_KEY=...    (faster, cheaper; console.groq.com/keys)")
