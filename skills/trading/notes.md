...
[2026-07-24] The sweep's per-symbol guard catches `(Exception, SystemExit)`, not just `Exception`. fetch_readings raises `SystemExit` via `lazy_import` when the finance stack (talib) is missing, and SystemExit subclasses BaseException, so a bare `except Exception` lets it escape and crash the whole 15-minute sweep before the blindness throttle downstream ever runs. A missing library is now treated like a data outage (counted failed, throttled to one ping per 4 hours).

[2026-07-24] `ensure_sweep_job` returns a tri-state (`"created"` / `"exists"` / `"error"`), not a bool. A bool conflated "already armed" with "scheduler unreachable", so a failed arm still read as success and cmd_add reported "Watching" over a sweep that would never run. cmd_add now warns the user when arming truly failed.

[2026-07-24] Tests import the trading scripts by bare name (`import watch`), which collides with the video-watch skill's `skills/watch/scripts/watch.py` under `unittest discover`. The trading test files evict any same-named module not loaded from `skills/trading/scripts` before importing, so the bind is deterministic regardless of test order.
