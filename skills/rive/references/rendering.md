...
- Measured on NVIDIA's own driver (Ubuntu 24.04, GTX 970, driver 580, CLI 1.1.1, 25 Sep 2026, and again on CLI 1.2.0 the same evening): captures draw through EGL on the NVIDIA device, `rive_doctor.py --web` passes, the web engine included (SwiftShader), and the live tests pass apart from the crash below.
- CLI 1.1.1 and 1.2.0 on Linux segfault (exit -11) under `--data-dump-every` once a key press is in the run: a key the scene handles (the button template's Enter, Rive's own keyboard_menu sample), or any key followed by a drag. Without a key, and with the same arguments under `--screenshot` or an end-only `--data-dump`, it runs clean. No script here dumps per frame with keys; the live timing test skips a key-bearing time on Linux when the CLI crashes on it, and a crashed capture reports "the CLI crashed (SIGSEGV)".
